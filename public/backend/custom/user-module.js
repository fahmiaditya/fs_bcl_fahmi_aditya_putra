...
------------------------------------------------- LAIN-LAIN -------------------------------------------------
===========================================================================================================*/
modul_layout.find('#search').attr('placeholder', 'Nama, Email');
...
function formIndex(tipe) {
    ...
    if (tipe == 0) {
        ...
        // DEFAULT INDEX
        $('#limo-title').html('Data User');
        $('#limo-breadcrumb').html('Data User');
        ...
        if (tipe == 1) {
            // DEFAULT INDEX
            $('#limo-title').html('Tambah User');
            $('#limo-breadcrumb').html('Tambah User');
            ...
            // DEFAULT INDEX
            $('#limo-title').html('Ubah User');
            $('#limo-breadcrumb').html('Ubah User');
...
loadRole();
function loadRole() {
    var url = modul_layout.find('#url_role_filter').val();
    ...
    $.ajax({
        ...
        success: function (resp) {
            // console.log(resp);
            modul_layout.find('#filter_role').html(resp);
        },
        complete: function () { // Set our complete callback, adding the .hidden class and hiding the spinner.
            modul_layout.find('#filter_role').val(0).trigger('change');
        },
        error: function(e) {
            Swal.fire({
                icon: "error",
...
function loadData(url) {
    ...
    var value_role = modul_layout.find('#filter_role').val();

    var other_data = $('#group_search :input').serializeArray();
    $.each(other_data,function(key,input){
        formData.append(input.name, input.value);
    });
    formData.append('filter', value_role);
...
function create(url) {
    $.ajax({
        ...
        // console.log(resp);
        modul_proses.find('#view-roles').html(resp);
...
function edit(url) {
    $.ajax({
        ...
        // console.log(resp);
        modul_proses.find('#view-roles').html(resp.roles);
        modul_proses.find('#view-permission').html(resp.permissions);
        updated_id = resp.data.key; // default update

        modul_proses.find('#name').val(resp.data.name);
        modul_proses.find('#username').val(resp.data.username);
        modul_proses.find('#email').val(resp.data.email);
        modul_proses.find('#telepon').val(resp.data.telepon);
        modul_proses.find('#roles').val(resp.data.hak_akses);
