...
use App\Repositories\JenisArmadaRepository;

class JenisArmadaService
{
    protected $jenisArmadaRepository;

    public function __construct(JenisArmadaRepository $jenisArmadaRepository)
    {
        $this->jenisArmadaRepository = $jenisArmadaRepository;
    }
    ...
    {
        $data       = $this->jenisArmadaRepository->getDataView($request);
    ...
    public function itemTable($data)
    {
        ...
        if (count($data->items()) > 0) {
            foreach ($data as $key => $item) {
                ...
                $url_update = $this->itemFormatOnClick(route('jenis-armada.update', $parameter).'/edit');

                // PENGECEKAN HAK AKSES EDIT DI IZINKAN
                $btn_update = '-';
                if (Auth()->user()->can('edit_jenis-armada')) {
    ...
    public function store($request)
    {
        ...
        if ($validator->passes()) {
            DB::beginTransaction();
            try {
                $this->jenisArmadaRepository->store($request);
                DB::commit();
    ...
    public function update($request, $id)
    {
        ...
        if ($validator->passes()) {
            ...
            try {
                $id = Crypt::decrypt($id);
                $this->jenisArmadaRepository->update($request, $id);
                DB::commit();
                return ['result' => Response::HTTP_CREATED];
            } catch (\Exception $e) {
                DB::rollback();
    ...
    public function delete($request)
    {
        ...
        try {
            foreach ($request->id as $item_id) {
                ...
                $this->jenisArmadaRepository->delete($id);
            }
